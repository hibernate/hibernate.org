...
You need to install CouchDB separately and configure the `COUCHDB_HOSTNAME` environment variable if you want to test that backend.
By default only the code of this module is compiled.

When running the build via maven, MongoDB is started and stopped automatically.
For running a test against MongoDB from within you IDE, MongoDB needs to be installed and started separately.

More build instructions are available in the project `readme.md` file.

=== Any detail on how the project is structured?

There are two types of tests in core

1. Unit tests for stuff in core itself
2. And the "backendtck" which are high-level (i.e. Session/EM-level) tests and which are executed for all backends.
   This is our backend TCK.
