...
That's the million dollars question but in essence, yes we thing it is a good fit.
Here is quickly a few key reasons why we thing JPA makes sense.
...
For better or worse, JPA is known to Java developers that are familiar with its API and semantic.
That's a huge win compared to learning an new lower level API or worse some proto-ORM level API.
