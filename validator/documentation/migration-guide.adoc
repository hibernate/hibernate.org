...
[[migration-guide-9-1-x]]
== 9.1.x

=== 9.1.0.Alpha1

While 9.1 is, in general, a drop-in replacement for 9.0, it may produce warnings at the metadata building stage
if cascading is applied to the container instead of the container type arguments.
It is a good time to finally move away from the Bean Validation 1.0/1.1 legacy approaches.
...
* Deprecated configuration property `hibernate.validator.constraint_mapping_contributor`
...
https://hibernate.atlassian.net/browse/HV-561[HV-561] introduced several deprecations (see the Javadoc for https://docs.hibernate.org/validator/4.3/api/index.html?deprecated-list.html[a complete deprecation list]):
