...
* *build-config*: the configuration for the build of HV
...
* *modules*: creates HV modules patches to be applied for HV module upgrades in WildFly
* *osgi*: the HV OSGi modules aggregator. Contains HV features for Apache Karaf and OSGi integration tests submodules
...
Hibernate Validator is using code style common to all Hibernate projects. Configuration xml files
