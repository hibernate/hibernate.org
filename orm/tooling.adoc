...
- the link:https://docs.hibernate.org/orm/{version_family}/userguide/html_single/#tooling-gradle[Gradle plugin] and link:https://docs.hibernate.org/orm/{version_family}/userguide/html_single/#tooling-maven[Maven plugin] which perform optional bytecode enhancement at build time.
...
Hibernate is able to infer a schema from mapping annotations or XML mappings and link:https://docs.hibernate.org/orm/7.0/introduction/html_single/#automatic-schema-export[automatically export the schema to the database], or verify that the inferred schema is consistent with the pre-existing schema.
This functionality is especially useful for testing.
The schema management tool can even truncate tables to link:https://docs.hibernate.org/orm/7.0/introduction/html_single/#testing[clean up between tests].
...
Your program can access this information link:https://docs.hibernate.org/orm/7.0/introduction/html_single/#statistics[via an API], but some environments report it automatically.
...
- link:https://docs.hibernate.org/orm/{version_family}/userguide/html_single/#appendix-monitoring-with-JFR[Hibernate JFR] reports persistence-related events to Java Flight Recorder.
